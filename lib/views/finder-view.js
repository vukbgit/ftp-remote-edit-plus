  ...
  constructor(treeView) {
    ...
    self.selectListView = new SelectListView({
      ...
      filterKeyForItem: (item) => {
        if (atom.config.get('ftp-remote-edit-plus.finder.filterKeyForItem') == "Filename") {
          return item.file;
        } else {
      ...
      atom.config.observe('remote-finder.useAlternateScoring', (newValue) => {
        ...
        if (this.useAlternateScoring) {
          this.selectListView.update({
            filter: (items, query) => {
              return query ? fuzzaldrinPlus.filter(items, query, { key: atom.config.get('ftp-remote-edit-plus.finder.filterKeyForItem') }) : items
            }
          })
        } else {
