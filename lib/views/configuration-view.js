...
export default class ConfigurationView extends ScrollView {
  ...
  constructor() {
    ...
    self.foldersView = new FoldersView();
    ...
    self.elements.append(self.createFolderSelect());
  ...
  createFolderSelect() {
    ...
    let selectLabelTitle = document.createElement('div');
    selectLabelTitle.textContent = 'Folder';
    selectLabelTitle.classList.add('setting-title');
    selectLabel.appendChild(selectLabelTitle);
    ...
    select.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        let serverIndex = self.selectServer.selectedOptions[0].value;
        self.configArray[serverIndex].folder = option.value;
      }
    });
  ...
  populateFolderSelect() {
      ...
      //load folders
      self.foldersView.password = self.password;
      var folders = self.foldersView.loadConfig();
console.log(folders);
      if (folders.length !== 0) {
        folders.forEach((folderItem, folderIndex) => {
          let option = document.createElement("option");
          option.value = folderItem.id;
          option.text = folderItem.name;
          self.selectFolder.add(option);
        });
      }
  }
  ...
  createServerSelect() {
    ...
    select.addEventListener('change', (event) => {
      if (self.configArray.length !== 0 && !self.disableEventhandler) {
        ...
        folders = self.selectFolder.children;
        selectedFolderIndex = null;
        for(folderIndex in folders) {
            option = folders[folderIndex];
            if(option.value == self.configArray[indexInArray].folder) {
                selectedFolderIndex = folderIndex;
                break;
            }
        };
        self.selectFolder.selectedIndex = selectedFolderIndex;
  ...
  loadConfig(loadConfig) {
    ...
    let configText = atom.config.get('ftp-remote-edit-plus.servers');
    let configArray = [];
  ...
  reload(loadConfig, selectedServer) {
    ...
    if (self.configArray.length !== 0) {
      ...
      for(folderIndex in folders) {
          option = folders[folderIndex];
          if(option.value == self.configArray[selectedIndex].folder) {
              selectedFolderIndex = folderIndex;
              break;
          }
      };
  ...
  enableInputFields() {
    ...
    self.selectFolder[0].classList.remove('disabled');
    self.selectFolder.disabled = false;
  ...
  disableInputFields() {
    const self = this;
    if(self.selectFolder.length > 0) {
        self.selectFolder[0].classList.add('disabled');
        self.selectFolder.disabled = true;
    }
  ...
  save(configArray) {
    ...
    if (self.configArray.length > 0) {
      atom.config.set('ftp-remote-edit-plus.servers', encrypt(self.password, JSON.stringify(self.configArray)));
    } else {
      atom.config.set('ftp-remote-edit-plus.servers', '');
    }
    self.close();
  }
