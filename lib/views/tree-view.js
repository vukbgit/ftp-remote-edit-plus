...
class TreeView extends ScrollView {
  ...
  static content() {
    return this.div({
      ...
      'data-show-on-right-side': atom.config.get('ftp-remote-edit-plus.tree.showOnRightSide'),
    }, () => {
      this.div({
  ...
  getDefaultLocation() {
    if (atom.config.get('ftp-remote-edit-plus.tree.showOnRightSide')) {
      return "right";
    } else {
      return "left";
    }
  ...
  initialize(state) {
    ...
    atom.config.onDidChange('ftp-remote-edit.tree.showOnRightSide', () => {
      self.element.dataset.showOnRightSide = atom.config.get('ftp-remote-edit-plus.tree.showOnRightSide');
      if (self.isVisible() && atom.config.get('ftp-remote-edit-plus.tree.showInDock') == false) {
        self.detach();
        self.attach();
      }
    });
  ...
  toggle() {
    const self = this;
    if (atom.config.get('ftp-remote-edit-plus.tree.showInDock')) {
  ...
  loadFolders(password) {
    ...
    let configHash = atom.config.get('ftp-remote-edit-plus.folders');
    if (configHash) {
      let config = decrypt(password, configHash);
      try {
        self.folders = JSON.parse(cleanJsonString(config));
        self.folders.sort((a, b) => {
          if (a.name.toLowerCase() < b.name.toLowerCase()) return -1;
          if (a.name.toLowerCase() > b.name.toLowerCase()) return 1;
          return 0;
        });
        //load servers
        self.loadServers(password);
        if(typeof self.servers != 'undefined') {
          //attach servers associated to a folder to the proper folder
          self.folders.forEach((config,index) => {
            self.folders[index].servers = self.servers[config.id];
          });
          //set folder NOT associated to any folder to be displayed
          self.servers = self.servers[null];
        }
        return true;
      } catch (e) {
  ...
  loadServers(password) {
    ...
    // Uncaught SyntaxError: Unexpected token  in JSON at position 0 #44
    // https://github.com/h3imdall/ftp-remote-edit/issues/44
    let configHash = atom.config.get('ftp-remote-edit-plus.servers');
    if (configHash) {
      let config = decrypt(password, configHash);
      let sortServerProfilesByName = atom.config.get('ftp-remote-edit-plus.tree.sortServerProfilesByName');

      try {
        servers = JSON.parse(cleanJsonString(config));
        self.servers = {};
        servers.forEach((config,index) => {
          //self.folders[index].servers = self.servers;
          if(!self.servers[config.folder]) {
              self.servers[config.folder] = [];
          }
          config.readyTimeout = atom.config.get('ftp-remote-edit-plus.tree.readyTimeout')
          self.servers[config.folder].push(config);
        });
  ...
  reload() {
    ...
    self.list.children().detach();
    self.hideInfo();
    if(self.folders && self.folders.length > 0) {
        self.folders.forEach((config) => {
