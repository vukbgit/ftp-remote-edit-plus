            ...
            if (self.editor === null || self.editor === undefined) {
              ...
              try {
                editor.onDidSave((saveObject) => {
                  ...
                  var activeElement = $(activePane.getElement());
                  var activeTabTitle = $(activeElement.find('li.active .title')[0]);
                  activeTabTitle.addClass('icon-sync').addClass('spin');
                  ...
                  editor.saveObject.getRoot()
                    ...
                    .then(() => {
                      if (atom.config.get('ftp-remote-edit-plus.notifications.showNotificationOnUpload')) {
                        editor.saveObject.getRoot()
                          .connector.showMessage('File successfully uploaded.', 'success');
                      }

                      if (foundInTreeView) {
                        // Remove sync icon from tree view
                        foundInTreeView.removeSyncFileIcon();
                      }
                      // Remove sync icon from file tab
                      activeTabTitle.removeClass('icon-sync').removeClass('spin');
                    })
                    .catch(function (err) {
