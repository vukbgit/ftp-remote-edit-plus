...
class FtpRemoteEdit {
  ...
  activate() {
    ...
    self.listeners.add(atom.commands.add('atom-workspace', {
      ...
      'ftp-remote-edit:edit-folders': () => self.editFolders(),
    ...
    // Events
    atom.config.onDidChange('ftp-remote-edit-plus.folders', () => {
      if (self.info.password) {
        //self.treeView.loadServers(self.info.password);
        self.treeView.loadFolders(self.info.password);
        self.treeView.reload(true);
      }
    });
    // Events
    atom.config.onDidChange('ftp-remote-edit-plus.servers', () => {
      if (self.info.password) {
        //self.treeView.loadServers(self.info.password);
        self.treeView.loadFolders(self.info.password);
        self.treeView.reload(true);
    ...
    if (atom.config.get('ftp-remote-edit-plus.tree.allowPendingPaneItems')) {
      atom.config.set('core.allowPendingPaneItems', true)
    }

    // Toggle on startup
    atom.packages.onDidActivatePackage((package) => {
      if (package.name == 'ftp-remote-edit-plus') {
        if (atom.config.get('ftp-remote-edit-plus.tree.toggleOnStartup')) {
          self.toggle();
        }
      }
    });
  ...
  changePassword(mode) {
    ...
    let promise = new Promise((resolve, reject) => {
      dialog.on('dialog-done', (e, passwords) => {
        ...
        if (mode == 'add') {
          let configHash = atom.config.get('ftp-remote-edit-plus.config');
          if (configHash) {
        ...
        self.changePassword('add')
          .then((returnValue) => {
            if (returnValue) {
              ...
              if (self.treeView.loadFolders(self.info.password)) {
  ...
  editFolders() {
    ...
    if (!self.info.password) {
      self.promtPassword(true)
        .then(() => {
          self.foldersView.password = self.info.password;
          self.foldersView.reload(true, root);
          self.foldersView.attach();
        });
      return;
    } else if (!self.foldersView.password) {
      self.foldersView.password = self.info.password;
    }
  ...
  autoRevealActiveFile(treeView) {
      //disabled for the moment since servers are not loaded at toggle
      return;
    const self = this;

    if (atom.config.get('ftp-remote-edit-plus.tree.autoRevealActiveFile')) {
